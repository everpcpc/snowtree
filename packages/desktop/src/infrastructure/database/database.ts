...
export class DatabaseService {
  ...
  private runMigrations(): void {
    ...
    // Timeline events table (command-level audit log for Git/CLI/Chat)
    const timelineEventsTable = this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='timeline_events'").all();
    if (timelineEventsTable.length === 0) {
      this.db.prepare(`
      ...
      `).run();
      this.db.prepare("CREATE INDEX idx_timeline_events_session_seq ON timeline_events(session_id, seq)").run();
      this.db.prepare("CREATE INDEX idx_timeline_events_session_ts ON timeline_events(session_id, timestamp)").run();
    } else {
      // This project treats timeline as an append-only audit log; it must be able to
      // record events that occur before a session row exists (e.g. worktree creation).
      const foreignKeys = this.db.prepare("PRAGMA foreign_key_list(timeline_events)").all() as Array<{ table?: string }>;
      const hasSessionsForeignKey = foreignKeys.some((fk) => fk.table === 'sessions');
      if (hasSessionsForeignKey) {
        this.transaction(() => {
          this.db.prepare("DROP TABLE timeline_events").run();
          this.db.prepare(`
            CREATE TABLE timeline_events (
              id INTEGER PRIMARY KEY AUTOINCREMENT,
              session_id TEXT NOT NULL,
