...
export class Logger {
  ...
  constructor(private configManager: ConfigManager) {
    this.logDir = getSnowtreeSubdirectory('logs');
  ...
  private getCurrentLogFileName(): string {
    ...
    return path.join(this.logDir, `snowtree-${date}.log`);
  }
  ...
  private rotateLogIfNeeded() {
    if (this.currentLogSize >= this.MAX_LOG_SIZE) {
      try {
        ...
        const rotatedFileName = path.join(this.logDir, `snowtree-${timestamp}.log`);
  ...
  private cleanupOldLogs() {
    try {
      const files = fs.readdirSync(this.logDir)
        .filter(file => file.startsWith('snowtree-') && file.endsWith('.log'))
        .map(file => ({
          name: file,
...
export function createLogger(configManager: ConfigManager): Logger {
