...
export const MainLayout: React.FC = React.memo(() => {
  ...
  const handleRequestPushPR = useCallback(async () => {
    ...
    const pushPrompt = [
      ...
      '   - git log -1 --oneline',
      '   - gh repo set-default --view  # Try to get default repo (preferred method)',
      '   - If gh repo set-default --view fails: use git remote -v to list remotes and extract <owner>/<repo> (prefer "upstream" if available, otherwise "origin")',
      '',
