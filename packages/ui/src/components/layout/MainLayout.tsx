...
export const MainLayout: React.FC = React.memo(() => {
  ...
  const handleRequestPushPR = useCallback(async () => {
    ...
    const pushPrompt = [
      ...
      '   - Extract <owner>/<repo> from remote URL (prefer "upstream" if available, otherwise "origin")',
      '   - Parse URL format: https://github.com/<owner>/<repo>.git OR git@github.com:<owner>/<repo>.git',
      '   - If selected repo remote is different from origin (e.g. "upstream"), then branch should be <origin-owner>:<branch-name> in gh commands',
      '',
      ...
      '   - Check existing: gh pr view --repo <owner>/<repo> <branch> --json number,url,state',
      `   - If no PR exists: gh pr create --repo <owner>/<repo> --draft --base ${baseBranch} --head <branch> --title "<title>" --body "<body>"`,
      '   - If PR exists: gh pr edit --repo <owner>/<repo> <branch> --title "<title>" --body "<body>" (only if needed)',
      '',
      'Guidelines:',
  ...
  const handleUpdateBranch = useCallback(async () => {
    ...
    const updatePrompt = [
      ...
      '     d. git add <file>  # Stage the resolved file',
      '   - git rebase --continue',
  ...
  const handleSyncPR = useCallback(async () => {
    ...
    const syncPrompt = [
      ...
      'Guidelines:',
      '- If working tree is dirty: stop and tell me to commit/stash changes first',
      '- If pull succeeds: report success',
